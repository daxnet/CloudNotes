...
namespace CloudNotes.WebAPI.Models.Filters
{
    ...
    public class WebApiAuthorizationAttribute : AuthorizationFilterAttribute
    {
        ...
        /// <returns></returns>
        protected virtual bool TryAuthorizeUser(string username, string password, HttpActionContext actionContext, out User user, ref List<string> roles)
        {
            user = null;
            roles.Clear();
            ...
            foreach (var role in user.Roles)
            {
                ...
                var permissions = permissionRepository.FindAll(Specification<Permission>.Eval(p => p.Role.ID == assignedRole.ID));
